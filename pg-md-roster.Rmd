...
pg_roster <- read_excel("Activation Tool Refresh_April 2026.xlsx", sheet = "Refresh")

pg_roster_processed <- pg_roster %>%
  mutate(NPI = trimws(gsub("'", "", `IT NPI`)),
         CLINIC_ID = as.numeric(trimws(gsub("'", "", `Epic Dept ID`)))) %>%
  mutate(pg_active = 1)
...
prov_list <- access_raw_tbl %>%
  ...
  filter(PROV_TYPE %in% prov_type_inc) %>%
...
merged_dept <- dept_list %>%
  ...
  left_join(pg_roster_processed %>%
              dplyr::select(`Epic Dept ID`, CLINIC_ID, pg_active) %>%
              distinct(),
...
<ul style="font-size:12pt; font-style:italic">
  <li>Activation Required: Department/ Provider has >10 visits in the last 3 months & currently not active in PG</li>
  <li>Deactivation Required: Department/ Provider has no visits in the last 12 months & currently active in PG</li>
  <li>No Activation Required: Department/ Provider has <10 visits in the last 3 months & currently not active in PG</li>
  <li>Active: Department/ Provider is currently active in PG</li>
</ul>
<p style="font-size:12pt; font-style:italic">Provider Types in Scope: `r prov_type_inc`</p>
